(function() {
  ...
  function isLocked(node) {
    return (node === document.pointerLockElement || node === document.mozPointerLockElement ||
      node === document.webkitPointerLockElement);
  }

  function isFullscreen(node) {
    return document.webkitFullscreenElement === node || document.mozFullscreenElement ===
      node || document.mozFullScreenElement === node;
  }

  function setup(component, node) {
    function request() {
      node.requestFullscreen = node.requestFullscreen || node.mozRequestFullscreen ||
